...
class Transportehorario(models.Model):
    ...
    def __str__(self):
        return self.origem + " - " + self.chegada + ' Horas: ' + str(self.horaChegada) + ' - ' + str(
            self.horaPartida) + ' ' + str(self.transporte)
    ...
    def get_trip_time(self):
        return datetime_time.strftime(self.horaPartida, "%H:%M") + ' - ' + datetime_time.strftime(self.horaChegada,
                                                                                                  "%H:%M")
...
#-----------------------------------------------------Nova tabela criada--------------------------------------------------
class TransporteGeral(models.Model):
    id = models.AutoField(db_column='ID', primary_key=True)
    transporte = models.CharField(db_column='Transporte', default='', max_length=255, blank=True)

    class Meta:
        db_table = 'TransporteGeral'

    def __str__(self):
        return str(self.transporte)
...
class Espaco(models.Model):
    ...
    edificio = models.ForeignKey('Edificio', models.CASCADE, db_column='Edificio', blank=True,
    ...
    def __str__(self):
        if self.edificio.image:
            return mark_safe('<a href=\'' + reverse('configuracao:verEdificioImagem',
                                                    kwargs={'id': self.edificio.id}) + '\'>' + escape(
                self.nome) + '</a>')
        else:
            return self.nome
...
class Edificio(models.Model):
    ...
    def __str__(self):
        if self.image:
            return mark_safe(
                '<a href=\'' + reverse('configuracao:verEdificioImagem', kwargs={'id': self.id}) + '\'>' + escape(
                    self.nome) + '</a>')
        else:
            return self.nome
